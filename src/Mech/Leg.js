...
export default class Leg extends GameObject {
	...
	update() {
		...
		let frontFootPosTarget = new Point({x: 10, y: 0}).add(this.parent.position);
		let rearFootPosTarget = new Point({x: -10, y: 0}).add(this.parent.position);
		...
		this.gunPosition(this.position.add({
			x: facing * 20,
			y: 20
		}), facing);
	}
	...
	ik(target : Point, facing : Facing = FACING_LEFT, sprites : {
		...
		if (endpoint.y > floor) {
			let ratio = (floor - this.position.y) / (endpoint.y - this.position.y);
			dist *= ratio;
