...
export default class Player extends Actor {
	...
	updateGrapple() {
		...
		if (hand.state == HAND_STATE.FIRED) {
			hand.position.x += Math.cos(hand.direction) * this.engine.deltaTime * hand.speed;
			hand.position.y += Math.sin(hand.direction) * this.engine.deltaTime * hand.speed;
	...
	updateGuns() {
		if (this.engine.input.getButton("fire")) {
			...
			this.engine.register(new Bullet({
				position: gunPoint,
				owner: this,
				time: 8,
				h: Math.cos(dir) * 10,
	...
	updateMissile() {
		if (missile.reload > 0) {
			...
			if (this.engine.input.getButton("special") && missile.energy >= missile.cost) {
				missile.reload = missile.reloadTime;
				missile.energy -= missile.cost;
				missile.regenSpeed = missile.regenBaseSpeed;
				// missile = false;
				this.engine.register(new Missile({
					owner: this,
					direction: -Math.PI / 2 + (Math.random() - 0.5) + this.leg.facing,
					speed: 10 + Math.random() * 5,
					position: this.leg.missileBarrelPos,
					target: this.engine.mouse.point.add(new Point({
						x: (Math.random() - 0.5) * 20,
						y: (Math.random() - 0.5) * 20
					}))
				}));
			} else {
				missile.regenSpeed += missile.regenSpeedIncrease * this.engine.deltaTime;
	...
	getGamePad() {
		...
		if (this.engine.input.getDevice() == "gamepad") {
			this.engine.mouse.point = this.position.add({
				x: 0,
				y: -this.size.h / 2
			}).add({
				x: gp.axes[0] * 300,
